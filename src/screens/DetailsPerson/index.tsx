...
import { EpisodesProps, RootStackParamList } from "../../utils/types";
export function DetailsPerson({ navigation }:any) {
  const [episodes, setEpisodes] = useState<EpisodesProps[]>([]);

  const { favorites, setFavorites } = useContext(FavoriteContext);
  const router = useRoute<RouteProp<RootStackParamList,"DetailsPerson" >>();
      ...
      navigation.setOptions({
        ...
        headerLeft: () => (
          <ButtonGoBack onPress={() => navigation.goBack()}>
            <ArrowLogo width={20} height={20} />
          </ButtonGoBack>
        ),
        headerRight: () => (
          <ButtonFavorite onPress={() => handleFavoritePerson()}>
            <HeartLogo
              width={20}
              height={20}
  ...
  const handleFavoritePerson = async () => {
    ...
    await AsyncStorage.setItem(
      "idsFavorites",
      JSON.stringify([router.params?.id])
    );
  };
  ...
  useEffect(() => {
    async function getDetailsPerson() {
      ...
      const numbersAfterEpisodes = router.params?.episode
  ...
  return (
    <Container>
      ...
      <About>
        ...
        <Location>Location: {router.params?.location.name}</Location>
      </About>
